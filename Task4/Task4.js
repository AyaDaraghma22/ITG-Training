...
// Check if the movie is already in favorites else If the movie is not in favorites, add it
async function addTofavorites(id) {
    console.log("fav-item", id);
    const isMovieInFavorites = Object.values(localStorage).includes(id);
...
//related movie will be displayed and that list is fetched when searching to movie.
...
// Function to handle Enter key press If the input is empty, show an alert & clear input after search
function handleKeyPress(event) {
    if (event.key === 'Enter') {
        const inputElement = document.getElementById('Input');
        const movieName = inputElement.value.trim();

        if (!movieName) {
            alert('This input is not valid.');
        } else {
            console.log('Entered movie:', movieName);
        }
